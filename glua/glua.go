...
Available options are:
  ...
  -v       show version information`)
	...
	if len(opt_c) == 0 && len(opt_e) == 0 && !opt_i && !opt_v && flag.NArg() == 0 {
		opt_i = true
	}
	...
	if opt_c != "" {
		...
		tmpLuaFileName := fmt.Sprintf("%d.lua", time.Now().UTC().Second())
		if opt_dt || opt_dc {
			chunk, err2 := parse.Parse(bytes.NewBufferString(opt_c), tmpLuaFileName)
			if err2 != nil {
			...
			if opt_dc {
				proto, err3 := lua.Compile(chunk, tmpLuaFileName)
				if err3 != nil {
		...
		if err := L.DoString(opt_c); err != nil {
			fmt.Println(err.Error())
			return 1
		}
	} else if nargs := flag.NArg(); nargs > 0 {
		script := flag.Arg(0)
		argtb := L.NewTable()
		for i := 1; i < nargs; i++ {
